  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        } else if (result.error === "A user with this email already exists.") {
        ...
        <div className="bg-white py-8 px-4 shadow-2xl rounded-2xl sm:px-10 border border-green-200">
          ...
          <div className="mt-6 text-center">
            ...
            >
              {authMode === 'register' ? (
                <>
                  <LogIn className="mr-2 h-5 w-5" /> Already have an account? Sign In
                </>
              ) : (
                <>
                  <UserPlus className="mr-2 h-5 w-5" /> Don't have an account? Register
                </>
              )}
            </button>
          </div>
